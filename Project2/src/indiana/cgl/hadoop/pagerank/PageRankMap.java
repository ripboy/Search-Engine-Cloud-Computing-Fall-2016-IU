...
public class PageRankMap extends Mapper < LongWritable, Text, LongWritable, Text > {
 ...
 InterruptedException {
  ...
  if (rrd.targetUrlsList.size() <= 0) {
   ...
   // Traverse the targetUrlsList to calculate the rank value per target URL
   for (Integer n: rrd.targetUrlsList) {
    Double rank = new Double(rrd.rankValue / rrd.targetUrlsList.size());
    context.write(new LongWritable(n), new Text(rank.toString()));
    // Build the string of # + targetURLs
