...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
		...
		byte[] pageBytes = Bytes.toBytes(pageDocId);
	        Get getData = new Get(pageBytes);
	        Result dataRow = dataTable.get(getData);
		pageUri = Bytes.toString(dataRow.getValue(Constants.CF_DETAILS_BYTES,Constants.QUAL_URI_BYTES));
            // Step 3: get the page rank value of this page from clueWeb09PageRankTable
		Result pageRankRow = prTable.get(getData);
			for (KeyValue pkv : pageRankRow.list()){
				if (pkv.matchingFamily(Constants.CF_PAGERANK_BYTES)){
					pageRank = Bytes.toFloat(pkv.getQualifier());
				}
			}
		    // END of your code
